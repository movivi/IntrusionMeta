...
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_INTRUSIONS_ALL_WithOutliers.csv",row.names = F)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_INTRUSIONS_DIARY_WithOutliers.csv",row.names = F)
...
#Control condition nature (active/experimental vs. passive/no-task)
df$ComparisonType = as.factor(df$ComparisonType)
a = getTable_Mod1("Moderation: INTUNSP Comparison Type Active VS. Passive",df$ComparisonType,2)
allTable = rbind(allTable,a)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_INTRUSIONS_TASK_WithOutliers.csv",row.names = F)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_INTRUSIONS_QUESTIONNAIRE_WithOutliers.csv",row.names = F)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_EMOTION_WithOutliers.csv",row.names = F)
...
#time of manipulation
df$TimeOfManipulation = as.factor(df$TimeOfManipulation)
a = getTable_Mod1("Moderation: INTUNSP Time of Manipulation",df$TimeOfManipulation,2)
allTable = rbind(allTable,a)

#write moderation results
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_SYMPTOMS_WithOutliers.csv",row.names = F)
