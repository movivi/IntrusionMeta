...
getTable_Mod1 <- function(analysisName,modFactor,modMain) {
  if (modMain == 1){
    ...
    metaObject = rma.mv(yi=yi, V = vi, data=df, mods = ~factor(modFactor), random = ~ 1 |  StudyID/EffectSizeID, control=list(optimizer="nlminb", rel.tol=1e-8))
  }
  ...
  meta_mod = rma.mv(yi=yi, V = vi, data=df, mods = ~factor(modFactor)-1, random = ~ 1 |  StudyID/EffectSizeID, control=list(optimizer="nlminb", rel.tol=1e-8))
...
##############################
# INTRUSION UNSPECIFIED SUBSET      #
##############################
allTable = getTemplate()

###Analysis for Intrusion All 
df = read.csv(IntrusionFile)
df = subset(df, outliers!=1) #remove outliers
...
a = getTable_Mod("Moderation: INTUNSP LEVEL 2 CATEGORY",df$Level2_final,2)
allTable = rbind(allTable,a)

#Level 3 beh/neuro/pharma
df$Level3_Beh_Pharma_Neuro = factor(df$Level3_Beh_Pharma_Neuro)
a = getTable_Mod("Moderation: INTUNSP LEVEL 3 BEH/NEURO/PHARMA",df$Level3_Beh_Pharma_Neuro,2)
allTable = rbind(allTable,a)

#Level 3 category
df$Level3_Imagery_Verbal_Emotion_Other = factor(df$Level3_Imagery_Verbal_Emotion_Other)
a = getTable_Mod("Moderation: INTUNSP LEVEL 3 CATEGORY",df$Level3_Imagery_Verbal_Emotion_Other,2)
allTable = rbind(allTable,a)

#Level 3 DIRECT/INDIRECT
df$Level3_Direct_Indirect = factor(df$Level3_Direct_Indirect)
a = getTable_Mod("Moderation: INTUNSP LEVEL 3 DIRECT/INDIRECT",df$Level3_Direct_Indirect,2)
allTable = rbind(allTable,a)

#Level 3 Task/Instruction
df$Level3_Task_Instruction = factor(df$Level3_Task_Instruction)
a = getTable_Mod("Moderation: INTUNSP LEVEL 3 task/instruction",df$Level3_Task_Instruction,2)
allTable = rbind(allTable,a)

#TraumaInductionStimuli
df$TraumaStimuliType = factor(df$TraumaStimuliType)
a = getTable_Mod("Moderation: INTUNSP Trauma induction stimuli",df$TraumaStimuliType,2)
allTable = rbind(allTable,a)

#Control condition nature (active/experimental vs. passive/no-task)
df$ComparisonType = as.factor(df$ComparisonType)
a = getTable_Mod("Moderation: INTUNSP Comparison Type Active VS. Passive",df$ComparisonType,2)
allTable = rbind(allTable,a)

#study design (between vs. within)
df$StudyDesign = as.factor(df$StudyDesign)
a = getTable_Mod("Moderation: INTUNSP Study design Between vs. Within",df$StudyDesign,2)
allTable = rbind(allTable,a)

#intrusion measurement mode
df$ModeOfMeasurement = as.factor(df$ModeOfMeasurement)
a = getTable_Mod("Moderation: INTUNSP ModeOfMeasurement",df$ModeOfMeasurement,2)
allTable = rbind(allTable,a)

#neuroimaging/psychophysiological measurement yes vs. no
df$Physio_binary = as.factor(df$Physio_binary)
a = getTable_Mod("Moderation: INTUNSP PhysioMeasures",df$Physio_binary,2)
allTable = rbind(allTable,a)

#publication status
df$PublicationStatus = as.factor(df$PublicationStatus)
a = getTable_Mod("Moderation: INTUNSP PublicationStatus",df$PublicationStatus,2)
allTable = rbind(allTable,a)
...
a = getTable_Mod("Moderation: INTUNSP Country",df$CountryofOrigin,2)
allTable = rbind(allTable,a)

#continent
df$Continent = as.factor(df$Continent)
a = getTable_Mod("Moderation: INTUNSP Continent",df$Continent,2)
allTable = rbind(allTable,a)
...
a = getTable_Mod("Moderation: INTUNSP Time of Manipulation",df$TimeOfManipulation,2)
allTable = rbind(allTable,a)

#write moderation results
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_INTRUSIONS_ALL_WithoutOutliers.csv",row.names = F)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_INTRUSIONS_DIARY_WithoutOutliers.csv",row.names = F)
...
#Level 3 beh/neuro/pharma THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$Level3_Beh_Pharma_Neuro = factor(df$Level3_Beh_Pharma_Neuro)
#a = getTable_Mod("Moderation: INTUNSP LEVEL 3 BEH/NEURO/PHARMA",df$Level3_Beh_Pharma_Neuro,2)
#allTable = rbind(allTable,a)
...
df$StudyDesign = as.factor(df$StudyDesign)
a = getTable_Mod1("Moderation: INTUNSP Study design Between vs. Within",df$StudyDesign,2)
allTable = rbind(allTable,a)
...
#neuroimaging/psychophysiological measurement yes vs. no THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$Physio_binary = as.factor(df$Physio_binary)
#a = getTable_Mod("Moderation: INTUNSP PhysioMeasures",df$Physio_binary,2)
#allTable = rbind(allTable,a)

#publication status THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$PublicationStatus = as.factor(df$PublicationStatus)
#a = getTable_Mod("Moderation: INTUNSP PublicationStatus",df$PublicationStatus,2)
#allTable = rbind(allTable,a)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_INTRUSIONS_TASK_WithoutOutliers.csv",row.names = F)
...
#Level 3 DIRECT/INDIRECT THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$Level3_Direct_Indirect = factor(df$Level3_Direct_Indirect)
#a = getTable_Mod("Moderation: INTUNSP LEVEL 3 DIRECT/INDIRECT",df$Level3_Direct_Indirect,2)
#allTable = rbind(allTable,a)
...
#TraumaInductionStimuli THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$TraumaStimuliType = factor(df$TraumaStimuliType)
#a = getTable_Mod("Moderation: INTUNSP Trauma induction stimuli",df$TraumaStimuliType,2)
#allTable = rbind(allTable,a)
...
#country
df$CountryofOrigin = as.factor(df$CountryofOrigin)
a = getTable_Mod1("Moderation: INTUNSP Country",df$CountryofOrigin,2)
allTable = rbind(allTable,a)

#continent THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$Continent = as.factor(df$Continent)
#a = getTable_Mod("Moderation: INTUNSP Continent",df$Continent,2)
#allTable = rbind(allTable,a)
...
#time of manipulation THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$TimeOfManipulation = as.factor(df$TimeOfManipulation)
#a = getTable_Mod("Moderation: INTUNSP Time of Manipulation",df$TimeOfManipulation,2)
#allTable = rbind(allTable,a)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_INTRUSIONS_QUESTIONNAIRE_WithoutOutliers.csv",row.names = F)
...
#Level 2 category
df$Level2_final = factor(df$Level2_final)
a = getTable_Mod1("Moderation: INTUNSP LEVEL 2 CATEGORY",df$Level2_final,2)
allTable = rbind(allTable,a)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_EMOTION_WithoutOutliers.csv",row.names = F)
...
#TraumaInductionStimuli
df$TraumaStimuliType = factor(df$TraumaStimuliType)
a = getTable_Mod1("Moderation: INTUNSP Trauma induction stimuli",df$TraumaStimuliType,2)
allTable = rbind(allTable,a)
...
#continent
df$Continent = as.factor(df$Continent)
a = getTable_Mod1("Moderation: INTUNSP Continent",df$Continent,2)
allTable = rbind(allTable,a)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTUNSP_SYMPTOMS_WithoutOutliers.csv",row.names = F)
