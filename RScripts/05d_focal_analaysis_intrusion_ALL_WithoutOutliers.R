...
df <- read.csv(TaskFile) #CHANGE FILENAME BASED ON THE DATASET YOU WANT TO RUN.
...
a = getTable_Mod("Moderation: INTALL_TASK LEVEL 3 BEH/NEURO/PHARMA",df$Level3_Beh_Pharma_Neuro,2) #CHANGE NAME BASED ON DATAFRAME USED
allTable = rbind(allTable,a)

#Level 3 category
df$Level3_Imagery_Verbal_Emotion_Other = factor(df$Level3_Imagery_Verbal_Emotion_Other)
a = getTable_Mod("Moderation: INTALL_TASK LEVEL 3 CATEGORY",df$Level3_Imagery_Verbal_Emotion_Other,2) #CHANGE NAME BASED ON DATAFRAME USED
allTable = rbind(allTable,a)

#Level 3 direct/indirect
df$Level3_Direct_Indirect = factor(df$Level3_Direct_Indirect)
a = getTable_Mod("Moderation: INTALL_TASK LEVEL 3 DIRECT/INDIRECT",df$Level3_Direct_Indirect,2) #CHANGE NAME BASED ON DATAFRAME USED
allTable = rbind(allTable,a)

#Level 3 Task/Instruction
df$Level3_Task_Instruction = factor(df$Level3_Task_Instruction)
a = getTable_Mod("Moderation: INTALL_TASK LEVEL 3 task/instruction",df$Level3_Task_Instruction,2)
allTable = rbind(allTable,a)

#write moderation results
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTALL_TASK_ALL_WithoutOutliers.csv",row.names = F) #cHANGE FILENAME BASED ON DATAFRAME USED
...
write.csv(meta_level2, "Results_RM_Meta-Analysis_INTALL_TASK_WithoutOutliers.csv",row.names = F) #CHANGE FILE NAME HERE BASED ON THE DATASET USED
