...
a = getTable_Mod("Moderation: INTINC LEVEL 2 CATEGORY",df$Level2_final,2)
allTable = rbind(allTable,a)

#Level 3 beh/neuro/pharma
df$Level3_Beh_Pharma_Neuro = factor(df$Level3_Beh_Pharma_Neuro)
a = getTable_Mod("Moderation: INTINC LEVEL 3 BEH/NEURO/PHARMA",df$Level3_Beh_Pharma_Neuro,2)
allTable = rbind(allTable,a)

#Level 3 category
df$Level3_Imagery_Verbal_Emotion_Other = factor(df$Level3_Imagery_Verbal_Emotion_Other)
a = getTable_Mod("Moderation: INTINC LEVEL 3 CATEGORY",df$Level3_Imagery_Verbal_Emotion_Other,2)
allTable = rbind(allTable,a)

#Level 3 DIRECT/INDIRECT
df$Level3_Direct_Indirect = factor(df$Level3_Direct_Indirect)
a = getTable_Mod("Moderation: INTINC LEVEL 3 DIRECT/INDIRECT",df$Level3_Direct_Indirect,2)
allTable = rbind(allTable,a)

#Level 3 Task/Instruction
df$Level3_Task_Instruction = factor(df$Level3_Task_Instruction)
a = getTable_Mod("Moderation: INTINC LEVEL 3 task/instruction",df$Level3_Task_Instruction,2)
allTable = rbind(allTable,a)

#TraumaInductionStimuli
df$TraumaStimuliType = factor(df$TraumaStimuliType)
a = getTable_Mod("Moderation: INTINC Trauma induction stimuli",df$TraumaStimuliType,2)
allTable = rbind(allTable,a)

#Control condition nature (active/experimental vs. passive/no-task)
df$ComparisonType = as.factor(df$ComparisonType)
a = getTable_Mod("Moderation: INTINC Comparison Type Active VS. Passive",df$ComparisonType,2)
allTable = rbind(allTable,a)

#study design (between vs. within)
df$StudyDesign = as.factor(df$StudyDesign)
a = getTable_Mod("Moderation: INTINC Study design Between vs. Within",df$StudyDesign,2)
allTable = rbind(allTable,a)

#intrusion measurement mode
df$ModeOfMeasurement = as.factor(df$ModeOfMeasurement)
a = getTable_Mod("Moderation: INTINC ModeOfMeasurement",df$ModeOfMeasurement,2)
allTable = rbind(allTable,a)

#neuroimaging/psychophysiological measurement yes vs. no
df$Physio_binary = as.factor(df$Physio_binary)
a = getTable_Mod("Moderation: INTINC PhysioMeasures",df$Physio_binary,2)
allTable = rbind(allTable,a)

#publication status
df$PublicationStatus = as.factor(df$PublicationStatus)
a = getTable_Mod("Moderation: INTINC PublicationStatus",df$PublicationStatus,2)
allTable = rbind(allTable,a)
...
a = getTable_Mod("Moderation: INTINC Country",df$CountryofOrigin,2)
allTable = rbind(allTable,a)

#continent
df$Continent = as.factor(df$Continent)
a = getTable_Mod("Moderation: INTINC Continent",df$Continent,2)
allTable = rbind(allTable,a)
...
write.csv(allTable,"Results_ModeratorAnalysis_INTINC_INTRUSIONS_ALL_WithoutOutliers.csv",row.names = F)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTINC_INTRUSIONS_DIARY_WithoutOutliers.csv",row.names = F)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTINC_INTRUSIONS_TASK_WithoutOutliers.csv",row.names = F)
...
#Level 3 Task/Instruction THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$Level3_Task_Instruction = factor(df$Level3_Task_Instruction)
#a = getTable_Mod("Moderation: INTINC LEVEL 3 task/instruction",df$Level3_Task_Instruction,2)
#allTable = rbind(allTable,a)
...
#Control condition nature (active/experimental vs. passive/no-task) THIS MODERATOR NOT VALID FOR THIS ANALYSIS
#df$ComparisonType = as.factor(df$ComparisonType)
#a = getTable_Mod("Moderation: INTINC Comparison Type Active VS. Passive",df$ComparisonType,2)
#allTable = rbind(allTable,a)
...
#country
df$CountryofOrigin = as.factor(df$CountryofOrigin)
a = getTable_Mod1("Moderation: INTINC Country",df$CountryofOrigin,2)
allTable = rbind(allTable,a)
...
#total IBA #MANUAL CHANGES MADE TO CONVERGENCE OPTIMAZATION: https://www.metafor-project.org/doku.php/tips:convergence_problems_rma
df$TotalIBAScore = as.numeric(df$TotalIBAScore)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTINC_INTRUSIONS_QUESTIONNAIRE_WithoutOutliers.csv",row.names = F)
...
#continent
df$Continent = as.factor(df$Continent)
a = getTable_Mod1("Moderation: INTINC Continent",df$Continent,2)
allTable = rbind(allTable,a)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTINC_EMOTION_WithoutOutliers.csv",row.names = F)
...
allTable <- sapply(allTable, as.character)
allTable[is.na(allTable)] <- ""
write.csv(allTable,"Results_ModeratorAnalysis_INTINC_SYMPTOMS_WithoutOutliers.csv",row.names = F)
